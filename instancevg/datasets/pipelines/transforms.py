    ...
    def _resize_mask(self, results):
        if results["with_mask"]:
            if self.keep_ratio:
                ...
                if "gt_mask_parts" in results:
                    results["gt_mask_parts"] = [part.resize(results["img_shape"][:2]) for part in results["gt_mask_parts"]]
            results["gt_mask_rle"] = maskUtils.encode(numpy.asfortranarray(results["gt_mask"].masks[0]))
            if "gt_mask_parts" in results:
                results["gt_mask_parts_rle"] = [maskUtils.encode(numpy.asfortranarray(part.masks[0])) for part in results["gt_mask_parts"]]
